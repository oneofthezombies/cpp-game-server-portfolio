...
public:
  ...
  public:
    ...
    [[nodiscard]] auto
    Build(const uint16_t port) const noexcept -> Result<MainEventLoopLinux>;
  };
  ...
  [[nodiscard]] auto
  Init(const std::string_view name) noexcept -> Result<Void> override;

private:
  explicit MainEventLoopLinux(FileDescriptorLinux &&server_fd) noexcept;

  [[nodiscard]] auto
  OnMailReceived(const Mail &mail) noexcept -> Result<Void> override;

  [[nodiscard]] auto OnEpollEventReceived(
      const struct epoll_event &event) noexcept -> Result<Void> override;
